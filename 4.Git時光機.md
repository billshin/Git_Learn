...
📌 `reflog` 是 Git 本地的黑盒子紀錄,但請注意是有保留天數的
| 類型             | 保存時間   |
|-----------------|-----------|
| 一般 commit      | 約 90 天  |
| 被刪除的分支     | 約 30 天  |
...
**revert 在幹嘛？ 為什麼不用reset?**  
因為 **remote 的 commit 只能往前**，就算錯誤，也需要保留歷史紀錄。  
所以 revert 是新增一個「否定變更」的 commit，而不是刪掉歷史。
...
# 1 指的是parent 1 , 通常是main
# 2 指的是parent 2 , 通常是feature branch
```

```mermaid
flowchart TD
    A[A --- B --- C --- D<br/>已 push 到 remote] --> B{D 有問題?}

    B -- Yes --> C1[git revert D]
    C1 --> D[A --- B --- C --- D --- D'<br/>新增 revert commit<br>雖然是新的commit,<br>內容已經跟C一樣]
...
還沒 push，本地 revert 後發現根本不用 revert：
- 可以直接 reset --hard HEAD~1，把 revert 移除

已經 push：
- remote 不應被改寫，只能再 revert 一次後push
