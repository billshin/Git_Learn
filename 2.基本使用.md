...
flowchart TD
    ...
    B --> C[Working Directory<br/>編輯 / 新增檔案]
    C --> D[git status<br/>查看狀態]

    D -->|有未追蹤或修改檔案| E[git add<br/>加入 Staging Area]
    E --> F[Staging Area<br/>準備提交的內容]

    F --> G[git commit -m<br/>建立 Commit]
    G --> H[Repository<br/>版本歷史保存]

    H --> I[git log<br/>查看 Commit 紀錄]
...
---
### git的資訊存放?
git 所有的訊息都放在`.git`這個資料夾 , 這是一個隱藏的資料夾
如果你去移除他,就會失去所有的紀錄
